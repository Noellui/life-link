...
const HOSPITAL_OPTIONS = [
  { id: 9001, name: 'City General Hospital'  },
  { id: 9002, name: 'Sterling Hospital'       },
  { id: 9003, name: 'SSG Hospital'            },
  { id: 9004, name: 'Baroda Medical College'  },
  { id: 9005, name: 'Bhailal Amin Hospital'   },
];
...
export default function RequestBlood() {
  ...
  const resolveHospitalId = () => {
    if (reqType.key === 'broadcast') return 9001;
      ...
      if (res.ok) {
        ...
        existing.unshift({
          ...
          hospitalName : reqType.isGlobal
            ...
            : (HOSPITAL_OPTIONS.find(h => h.id === hospId)?.name || 'City General'),
          ...
          <div style={{ ...S.successCard, animation:'fadeUp 0.35s ease' }}>
            ...
            {reqType.key === 'hospital' && (
              <div style={{ ...S.successNote, background:'#EFF6FF', color:'#1D4ED8', borderColor:'#BFDBFE' }}>
                🏥 Requisition sent to{' '}
                {HOSPITAL_OPTIONS.find(h => h.id === Number(form.hospitalId))?.name || 'the hospital'}.
              </div>
            )}
